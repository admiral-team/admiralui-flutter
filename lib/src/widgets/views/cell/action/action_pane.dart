...
class _ActionPaneState extends State<ActionPane> implements RatioConfigurator {
  ...
  double normalizeRatio(double ratio) {
    ...
    final double absoluteRatio = ratio.abs().clamp(0.0, widget.extentRatio);
    if (ratio < 0) {
  ...
  void handleEndGestureChanged() {
    final EndGesture? gesture = controller!.endGesture.value;
  ...
  void handleRatioChanged() {
    final bool show = controller!.ratio.abs() <= widget.extentRatio &&
  ...
  Widget build(BuildContext context) {
    final ActionPaneConfiguration config = ActionPaneConfiguration.of(context)!;

    Widget? child;

    if (showMotion) {
      final double factor = widget.extentRatio;
