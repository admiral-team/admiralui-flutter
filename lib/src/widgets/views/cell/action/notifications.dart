...
class SlidableGroupNotification {
  ...
  static SlidableGroupNotificationDispatcher<T>? createDispatcher<T>(
    ...
    final _InheritedSlidableNotification<T>? widget = context
  ...
  static void dispatch<T>(
    ...
    final SlidableGroupNotificationDispatcher<T>? dispatcher = 
    createDispatcher<T>(
      context,
...
class SlidableGroupNotificationDispatcher<T> {
  ...
  void dispatch(T notification) {
    final ValueNotifier<T?> notifier = _inheritedSlidableNotification.notifier;
    final T? Function(T)? onNotification = 
    _inheritedSlidableNotification.onNotification;
    final T? effectiveNotification =
...
class _SlidableGroupBehaviorState<T> extends State<SlidableGroupBehavior<T>> {
  final ValueNotifier<T?> valueNotifier = ValueNotifier<T?>(null);
...
class _SlidableGroupBehaviorListenerState<T>
  ...
  void didChangeDependencies() {
    ...
    final ValueNotifier<T?>? oldNotifier = notifier;
    final ValueNotifier<T?>? newNotifier 
    = _InheritedSlidableNotification.of<T>(context);
  ...
  void handleNotification() {
    final T? notification = notifier?.value;
